#include "testCaseBatchNorm_1.hxx"
#include <algorithm>
#include <iostream>
#include <chrono>
#include <stdlib.h>
#include <time.h>
#include <cmath>
#include <numeric>
//#include <vector>

int main(){

	#define FLASH_CACHE

   	const int n = 5;
	const int batchsize = 10;
   	std::cout << "batchsize=" << batchsize << std::endl;
   	float inputss[n][40*30*20*batchsize];
	// std::fill_n(inputss, 40*30*2*batchsize, 0.0);

   	for (int k = 0; k < n; ++k) {
		for (int i = 0; i < 40*30*20* batchsize; i++){
			srand(time(0));
			inputss[k][i] = rand();
		}
	}

	std::vector<float> total_time;
	const size_t bigger_than_cachesize = 10 * 1024 * 1024;

	for (int k = 0; k < n; ++k ) {

	#ifdef FLASH_CACHE

		std::vector<float> tmp(bigger_than_cachesize);
		// When you want to "flush" cache.
		for(int i = 0; i < bigger_than_cachesize; i++)
		{
			tmp[i] = rand();
		}

	#endif

		auto t1 = std::chrono::high_resolution_clock::now();
		auto out = TMVA_SOFIE_testCaseBatchNorm_1::infer(inputss[k]);
		auto t2 = std::chrono::high_resolution_clock::now();
		total_time.push_back(float(std::chrono::duration_cast<std::chrono::microseconds>( t2 - t1 ).count()));
	}

	float sum = std::accumulate(total_time.begin(), total_time.end(), 0.0);
	float mean = sum / total_time.size();

	float sq_sum = std::inner_product(total_time.begin(), total_time.end(), total_time.begin(), 0.0);
	float std = std::sqrt(sq_sum / total_time.size() - mean * mean);

	std::cout << std::endl << "mean : "<< mean<< std::endl;
	std::cout << "mean/ batchsize : " << mean/ batchsize  << std::endl;
	std::cout << "std/ batchsize : " << std/ batchsize  << std::endl;

}