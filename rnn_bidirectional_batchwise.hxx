...
float tensor_initialh[24] = {-0.371075004, 0.252532989, -1.42194998, 0.393029988, -0.0841611996, 1.45696998, 0.0679387003, 2.11547995, -0.463111997, -1.02437997, -0.538398981, -2.21508002, -1.51050997, 1.50948, 0.206350997, -0.981445014, -1.42209995, -0.149364993, 1.25870001, 1.38294005, -0.221477002, -0.230483994, 0.453312993, 0.795476019};
	...
	for(size_t seq = 0; seq < 3; seq++) {
		for(size_t batch = 0; batch < 3; batch++) {
			for(size_t i = 0; i < 2; i++) {
				op_0_input[seq * 6 + batch * 2 + i] = tensor_X[batch * 6 + seq * 2 + i];
			}
		}
	}
float op_0_initial_hidden_state[24];
	for(size_t batch = 0; batch < 3; batch++) {
		for(size_t h = 0; h < 4; h++) {
			op_0_initial_hidden_state[0 + batch * 4 + h] = tensor_initialh[batch * 8 + 0 + h];
		}
	}
	for(size_t batch = 0; batch < 3; batch++) {
		for(size_t h = 0; h < 4; h++) {
			op_0_initial_hidden_state[12 + batch * 4 + h] = tensor_initialh[batch * 8 + 4 + h];
		}
	}
	float op_0_feedforward[36];
	float op_0_hidden_state[72];
	char op_0_transA = 'N';
	char op_0_transB = 'T';
	int op_0_m = 9;
	int op_0_n = 4;
	...
	for (size_t seq = 0; seq < 3; seq++) {
		for (size_t batch = 0; batch < 3; batch++) {
			size_t offset = seq * 24 + 0 + batch * 4;
			size_t y_offset = batch * 24 + seq * 8 + 0;
			std::copy(op_0_hidden_state + offset, op_0_hidden_state + offset + 4, tensor_Y + y_offset);
		}
	}
	for (size_t seq = 0; seq < 3; seq++) {
		for (size_t batch = 0; batch < 3; batch++) {
			size_t offset = seq * 24 + 12 + batch * 4;
			size_t y_offset = batch * 24 + seq * 8 + 4;
			std::copy(op_0_hidden_state + offset, op_0_hidden_state + offset + 4, tensor_Y + y_offset);
		}
	}
	for (size_t batch = 0; batch < 3; batch++) {
