//Code generated automatically by TMVA for Inference of Model file [testCaseConv_1.onnx] at [Sat Jun 26 20:16:00 2021] 
#include<vector>
namespace TMVA_SOFIE_testCaseConv_1{
namespace BLAS{
...
std::vector<float> infer(float* tensor_x){
	float op_0_xpad[64] = {0};
	for (size_t n = 0; n < 1; n++) {
		for (size_t c = 0; c < 1; c++) {
			for (size_t h = 0; h < 5; h++) {
				for (size_t w = 0; w < 5; w++) {
					op_0_xpad[n * 64 + c * 64 + (h + 2) * 8 + w + 2] = tensor_x[n * 25 + c * 25 + h * 5 + w];
				}
			}
		}
	}
	float op_0_xcol[81] = {0};
	for (size_t g = 0; g < 1; g++) {
		size_t idx = g * 9;
		for (size_t n = 0; n < 1; n++) {
			for (size_t c = g * 1; c < (g + 1) * 1; c++) {
				for (size_t h = 0; h < 6; h += 2) {
					for (size_t w = 0; w < 6;w += 2) {
						for (size_t x = 0; x < 3; x++) {
							for (size_t y = 0; y < 3; y++) {
								op_0_xcol[idx] = op_0_xpad[n * 64 + c * 64 + (h + x) * 8 + w + y];
								idx++;
							}
						}
					}
